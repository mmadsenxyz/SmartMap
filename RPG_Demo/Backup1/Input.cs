...
namespace RolePlayingGame
{
    ...
    public class Input
    {
        ...
        private Form listener;
        private Dictionary<Keys, bool> listener_Table;

        public Input(Form target)
        {
            listener_Table = new Dictionary<Keys, bool>();
            foreach (Keys key in EnumUtil.GetValues<Keys>())
            {
                if (!listener_Table.ContainsKey(key))
                {
                    listener_Table.Add(key, false);
                }
            }
        ...
        {
            listener_Table[(Keys)e.KeyCode] = true;
        }

        private void listener_KeyUp(object sender, KeyEventArgs e)
        {
            listener_Table[(Keys)e.KeyCode] = false;
        }

        public bool IsKeyPressed(Keys key)
        {
            bool result = listener_Table[key];
            return result;
        }
    }
}
